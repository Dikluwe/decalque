...
**Substitui**: `00_nucleo/prompts/entities/page-geometry.md` (o `PageGeometry { width, height }` original é absorvido e estendido por este modelo; a spec antiga fica como histórico)
**ADR**: `00_nucleo/adr/0002-lopdf-backend-parsing.md` (secção "Geometria de página")
...
do pipeline consome (`CartesianOrigin::normalize`, `DocumentGeometry`, motor de comparação).

Tipo numérico: **`f64` em todos os campos** (ver `00_nucleo/adr/0003-convencoes-transversais.md`).

## Restrições estruturais

- L1: zero I/O. `PageBoxModel` é construído por `03_infra` a partir do dicionário de página
  lido via lopdf; `resolve_page_geometry` é função pura sobre ele.
...
pub enum PageGeometryDiagnostic {
    NonStandardRotation,   // Rotate não múltiplo de 90 após módulo 360 — tratado como Deg0
    InvalidUserUnit,       // UserUnit zero, negativo ou inválido — tratado como 1.0
}
...
2. `width` = largura da caixa efectiva (`x1 - x0`), `height` = altura (`y1 - y0`) — **antes da
   rotação**.
3. `Rotate`: normalizar com módulo 360 (`((r % 360) + 360) % 360`). Aceitar 0/90/180/270.
   Se o valor final não for múltiplo de 90, tratar como `Deg0` e emitir
   `PageGeometryDiagnostic::NonStandardRotation`. `None` → `Deg0`.
4. `UserUnit`: se não existir, `1.0`. Se existir e for positivo, armazenar. Se for zero,
   negativo ou inválido, usar `1.0` e emitir `PageGeometryDiagnostic::InvalidUserUnit`.
   O valor é **armazenado** mas não altera `width`/`height` na primeira versão — registar esta
   limitação no doc-comment do campo.
...
Manter `size_delta(&self, other) -> (f64, f64)` do modelo original (convenção `other - self`),
agora sobre o tipo resolvido.
...
Dado `user_unit: Some(0.0)`
Quando `resolve_page_geometry` é chamado
Então `user_unit == 1.0` e o diagnóstico `InvalidUserUnit` é emitido

Dado um `PageGeometry` com `rotation == Deg90` e `width != height`
Quando `display_size` é chamado
Então devolve `(height, width)`

Dado duas `PageGeometry` iguais / com larguras diferentes
Quando `size_delta` é chamado
Então comporta-se como na spec original (`(0.0, 0.0)`; delta com sinal, `other - self`)
