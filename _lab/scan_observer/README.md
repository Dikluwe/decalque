...
```sh
.venv-paddle/bin/python _lab/scan_observer/paddle_scan_observation.py pagina.png \
  --output pagina.scan-observation.json \
  --page-index 0 \
  --run-id livro-p0001 \
  --lang pt

cargo run --bin decalque -- validate-scan-observation pagina.scan-observation.json \
  --raster pagina.png
```
...
```sh
python3 _lab/scan_observer/block_ocr_pipeline.py _lab/scan_observer/config/ovis-first-page.json \
...
.venv-paddle/bin/pip install -r _lab/scan_observer/requirements.txt
...
```sh
.venv-paddle/bin/python _lab/scan_observer/paddle_lmstudio_adapter.py documento.png
```
...
```sh
.venv-paddle/bin/python _lab/scan_observer/paddle_line_detector.py documento.png > lines.json
python3 _lab/scan_observer/line_geometry_matcher.py scan.json lines.json > scan-with-lines.json
```
...
```sh
.venv-paddle/bin/python _lab/scan_observer/word_geometry_detector.py scan-with-lines.json documento.png > scan-with-words.json
```
...
```sh
python3 _lab/scan_observer/coordinate_transform.py scan-with-words.json candidate-fonts.json > scan-points.json
```
...
```sh
python3 _lab/scan_observer/typographic_profile.py scan-points.json > scan-profile.json
```
...
```sh
python3 _lab/scan_observer/candidate_raster_profile.py scan-profile.json candidate-fonts.json candidato.pdf > candidate-profile.json
```
...
```sh
python3 _lab/scan_observer/typography_corpus.py
python3 _lab/scan_observer/layout_corpus.py
python3 _lab/scan_observer/ocr_error_corpus.py
```
...
python3 _lab/scan_observer/candidate_font_matcher.py scan.json candidate-fonts.json > enriched.json
```
...
```sh
python3 _lab/scan_observer/scan_word_compare.py scan-points.json candidate-fonts.json > comparison.json
```
...
```sh
python3 _lab/scan_observer/local_font_discovery.py --evidence font-evidence.json \
...
```sh
python3 _lab/scan_observer/book_page_masters.py page-observations.json > book-page-masters.json
```
...
```sh
python3 _lab/scan_observer/book_spread_feedback.py ocr.json book-page-masters.json livro.pdf \
...
```sh
python3 _lab/scan_observer/line_diff_analyzer.py reference.png candidate.png \
...
```sh
python3 _lab/scan_observer/scan_font_builder.py glyph-samples.json output/book-derived.ttf \
...
```sh
python3 _lab/scan_observer/scan_font_builder.py glyph-samples.json output/book-derived-sdf.ttf \
...
python3 _lab/scan_observer/google_fonts_discovery.py innovation.png \
...
```sh
python3 _lab/scan_observer/google_fonts_discovery.py --evidence font-evidence.json \
...
```sh
python3 _lab/scan_observer/typst_reconstruction_pipeline.py page-manifest.json scan.pdf \
...
```sh
python3 _lab/scan_observer/editorial_typst_composer.py editorial/page-018.json page-018.png \
...
```sh
python3 _lab/scan_observer/editorial_book_composer.py editorial/ rendered-pages/ \
...
```sh
python3 _lab/scan_observer/typography_profile_trial.py livro.pdf \
...
```sh
python3 _lab/scan_observer/typst_feedback_optimizer.py page-manifest.json scan.pdf \
...
```sh
python3 _lab/scan_observer/dual_lmstudio_ocr.py pagina.png \
...
```sh
python3 _lab/scan_observer/visual_symbol_catalog.py output/paginas/page-*.json > symbols.json
```
...
```sh
python3 _lab/scan_observer/recurring_symbol_finder.py folha.png pagina-*.png \
...
.venv-paddle/bin/python _lab/scan_observer/scan_compare_pipeline.py documento.png candidato.pdf > result.json
```
...
```sh
.venv-paddle/bin/python _lab/scan_observer/scan_compare_document.py livro-scan.pdf livro-digital.pdf > result.json
```
